 ...
 */
 ...
 public static String name()         {  return "Smart Group Dimming"  }
metadata {
...
def setLevel(val) {
	if (detailedLog) log.debug "${device.displayName} setLevel command received, level: ${val}"
    sendCommand(setLevel, true, val)
}

private sendCommand(cmnd, trigger, arg = null) {
	if (detailedLog) log.debug "$device.displayName sendCommand command received: $cmnd"
	if (arg || arg == 0) {
    	sendEvent(name: "level", value: arg)
    } else {
    	sendEvent(name: "switch", value: cmnd)
    }
    if(trigger) { parent.groupCommand(cmnd, arg)}
}
...
private levelSync(values) {	
    if (detailedLog) log.debug "$device.displayName syncLevel command received values: $values"
    def val = Math.round(values.sum() / values.size())
    if (detailedLog) log.debug "Average Level: $val"
    sendCommand(setLevel, false, val)
}
